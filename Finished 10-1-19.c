...
void Insert_zero_after(List RearPtrL) {
    List p = (List)malloc(sizeof(struct LNode));
...
void Change(int item, int K, List PtrL) {
    Find_Kth(K, PtrL)->Data = item + '0';
}

void Insert(ElementType x, List RearPtrL) {
    List p = (List)malloc(sizeof(struct LNode));
...
void Insert_Kth(ElementType X, int K, List PtrL) {
    List p, s;
